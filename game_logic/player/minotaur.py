...
class Minotaur(Player):
    ...
    def move(self, vector: CoordPair = None, ticks: int = None):
        super().move(vector, ticks)
        # acceleration
        accel_mod = MinoAccelerationModifier(self.speed * 1e-2, ModifierType.Speed, vector)
    ...
    def update_modifiers(self, ticks: int):
        ...
        #     if not isinstance(spd_mod, MinoAccelerationModifier):
        #         continue
        #     if spd_mod.move_dir.angle_between(self.move_direction.normalize()) > constants.MINOTAUR_ACC_MAX_ANGLE:
